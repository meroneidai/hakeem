@props(['name' => 'image', 'path' => null, 'label' => null, 'hint' => null, 'required' => false])

<x-field :label="$label ?? __('common.image')" :name="$name" :hint="$hint ?? __('common.image_hint')" :required="$required" class="sm:col-span-2">
    @if ($path)
    ...
    <input
        ...
        @if ($required && ! $path) required @endif
        {{ $attributes->merge([
