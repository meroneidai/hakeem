<x-field :label="__('booking.day')" name="date">
    <div class="space-y-3">
        <div class="flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <template x-for="day in days" :key="day.value">
                <button
                    ...
                    @click="date = day.value; $dispatch('day-changed')"
                >
        ...
        <div>
            <label class="mb-1 block text-xs text-ink-500">{{ __('booking.other_date') }}</label>
            <input
                type="date"
                :value="date"
                :min="days[0]?.value"
